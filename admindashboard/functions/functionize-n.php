...
<?php 
	...
	$r = new mysql($mysqlvars); 
		 ...
		 $mainupline = $_POST['mainupline'];
		 
		 
		 
		 
		 	$row = mysql_query("SELECT count(pin) as pincount FROM HPWPINNE01 WHERE serial between '$PINFROM' and '$PINTO' ");
	...
	//	echo "$total[0]";	  
            ...
            if ( $total[0] > 0 ) {		 
			   ...
			   $totalci = mysql_query("SELECT COUNT(*) FROM HPWPINNE01 WHERE PIN='$pin[$i]'"); 
               $totalci = mysql_fetch_array($totalci); 	 
			//   echo "$totalci[0]";	
			   if ( $totalci[0] > 0 ) {	
               	  $row = mysql_query("SELECT PIN FROM HPWPINNE01 WHERE PIN = '$pin[$i]'");
                  $row = mysql_fetch_array($row);				      
				  $tpin = $row[0];
	...
	$treftots = $tref[0]; 
	 ...
	 $pincheck1 = mysql_query("SELECT serial FROM  `HPWPINNE01`    where pin = '$pin[$i]'");
	 $pincheck1= mysql_fetch_array($pincheck1); 	
	 $pincheck = $pincheck1[0];
	...
	$rtotal5th = 5;	
     ...
     $query1=mysql_query("call insertRBlevel('$usercode[$i]', '$mainupline','$rtotal',now())");
	 $query2=mysql_query("call insertRBlevel('$usercode[$i]', '$total2ndL1','$rtotal2nd',now())");
	 $query3=mysql_query("call insertRBlevel('$usercode[$i]', '$total3rdL1','$rtotal3rd',now())");
	 $query4=mysql_query("call insertRBlevel('$usercode[$i]', '$total4thL1','$rtotal4th',now())");
	 $query5=mysql_query("call insertRBlevel('$usercode[$i]', '$total5thL1','$rtotal5th',now())");

  
  
/* Register */
  $query=mysql_query("call Insertnewentry('$usercode[$i]', '$password', '$lastname', '$firstname', '$referral[$i]', now(),'$mname', '$pin[$i]')");
...
<script type="text/javascript">
      ...
      function myFunction() {
		...
		var netincome = document.getElementById("txt3").value;
        var x = (parseFloat(y) * parseFloat(z)); 
		
		var rb = ( parseFloat(netincome) - parseFloat(x)); 
          
        document.getElementById("price").innerHTML = x;
		document.getElementById("remainingbalance").innerHTML = rb;
		
		document.getElementById("totalamount").value = x
		document.getElementById("remainingbalanceinput").value = rb
	  ...
	  function getData() 
	  {
        ...
        var codes     = document.getElementById("noofcodes").value;
		var rhid = document.getElementById("rhid").value;
		
	  document.getElementById("fullname").value = fullnames
		document.getElementById("usercode").value = rhid
		...
		session.setItem('noofcodes', codes); 
    ...
    <div class="col-lg-12" style="padding-top:10px; float:left;">
      ...
      <form   enctype="multipart/form-data" action="" method="post">
	   <div class="form-group">
            <label>mainupline</label>
            <input class="form-control" name="mainupline" required placeholder="mainupline here">
          </div>
	      <div class="form-group">
        ...
        <div class="form-group">
		  ...
		  <p>referral</p>
            <select  name="referral[]"  multiple="multiple" class="form-control">
			  <option selected value="<?php echo $yesupline  b     ?>"> <?php echo $yesupline?></option>
              <?php 
 		...
 		$totalblue = "SELECT *  FROM HPWPINNE01 WHERE   serial between '$PINFROM' and '$PINTO'  ORDER BY SERIAL ASC";  
